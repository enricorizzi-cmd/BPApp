...
(function(){
      ...
      try{
        ...
        if (v>0){
          try{
            ...
            // apri builder (viewGI ascolta l'evento)
  ...
  // --- Crea/Aggiorna GI a partire dall’appuntamento e restituisce la vendita ---
  async function upsertGIFromAppointment(appt, vss){
    ...
    // normalizza vari possibili risposte
  ...
  // --- Scan appuntamenti (ultimi N giorni) ---
  async function scan(){
    try{
      ...
      // ordina dal più recente e valuta ogni appuntamento nella finestra
  ...
  // --- API pubbliche/boot ---
  window.scanBanners = scan;  // se vuoi rieseguire manualmente
  // avvio automatico dopo il ready (se non già cablato altrove)
