...
;(function () {
  ...
  function load(k,def){ var s=localStorage.getItem(k); if(s==null) return def; try{ return JSON.parse(s);}catch(e){ return s; } }
  function del(k){ localStorage.removeItem(k); }

  function setToken(t, remember){ if(remember){ save('bp_token', t);} else { sessionStorage.setItem('bp_token', t);} }
  function getToken(){ return localStorage.getItem('bp_token') || sessionStorage.getItem('bp_token') || ''; }
  function setUser(u){ save('bp_user', u); }
  function getUser(){ var u=load('bp_user',null); return u||null; }
  function logout(){ del('bp_token'); sessionStorage.removeItem('bp_token'); del('bp_user'); location.href='/?v=13'; }

  // ===== XP gamification =====
  function getXP(){ return parseInt(localStorage.getItem('bp_xp')||'0',10); }
  function addXP(n){ var x=getXP()+Number(n||0); if(x<0)x=0; localStorage.setItem('bp_xp', String(x)); rerenderTopbarSoon(); return x; }
  function level(){ var x=getXP(); return Math.floor(1+Math.sqrt(x/100)); }
...
function api(path, opts){
  ...
  // imposta Content-Type solo se c'è un body "non stringa"
...
return fetch(path, opts).then(async (r)=>{
  const text = await r.text();        // leggo sempre la response una volta sola
  if (!r.ok) {
    // tento di estrarre errore JSON dal server
    try { 
...
function GET(path, extra){ return api(path, Object.assign({ method:'GET' }, extra||{})); }
function POST(path, body){ return api(path, { method:'POST', body: body||{} }); }
function DEL(path){ return api(path, { method:'DELETE' }); } // opzionale

// ===== Date utils (it-IT) =====
function pad2(n){ return (n<10?'0':'')+n; }
function dmy(d){ const x=new Date(d); return pad2(x.getDate())+'/'+pad2(x.getMonth()+1)+'/'+x.getFullYear(); }
function ymd(d){ const x=new Date(d); return x.getFullYear()+'-'+pad2(x.getMonth()+1)+'-'+pad2(x.getDate()); }
function timeHM(d){ const x=new Date(d); return pad2(x.getHours())+':'+pad2(x.getMinutes()); }

// Settimana con lunedì come inizio
function startOfWeek(d){
  const x=new Date(d); x.setHours(0,0,0,0);
  const diff=(x.getDay()+6)%7; // 0=Mon
...
// Quarter/Semester/Year helpers
function startOfQuarter(d){ const x=new Date(d), q=Math.floor(x.getMonth()/3); return new Date(x.getFullYear(), q*3, 1); }
function endOfQuarter(d){ const s=startOfQuarter(d); return new Date(s.getFullYear(), s.getMonth()+3, 0, 23,59,59,999); }
function startOfSemester(d){ const x=new Date(d), s=(x.getMonth()<6?0:6); return new Date(x.getFullYear(), s, 1); }
function endOfSemester(d){ const s=startOfSemester(d); return new Date(s.getFullYear(), s.getMonth()+6, 0, 23,59,59,999); }
function startOfYear(d){ const x=new Date(d); return new Date(x.getFullYear(),0,1); }
function endOfYear(d){ const x=new Date(d); return new Date(x.getFullYear(),11,31, 23,59,59,999); }

// ISO week bounds by year/week
function startOfIsoWeek(y, w){
  const simple = new Date(y,0,1 + (w-1)*7);
  const dow = simple.getDay() || 7; // Sun -> 7
...
function nextMonthBounds(from){ const x=new Date(from.getFullYear(), from.getMonth()+1, 1); return {start:x, end:new Date(x.getFullYear(), x.getMonth()+1, 0, 23,59,59,999)}; }
function prevMonthBounds(from){ const x=new Date(from.getFullYear(), from.getMonth(), 0, 23,59,59,999); return {start:new Date(x.getFullYear(), x.getMonth(), 1), end:x}; }
function nextQuarterBounds(from){ const s=startOfQuarter(new Date(from.getFullYear(), from.getMonth()+3, 1)); return {start:s, end:new Date(s.getFullYear(), s.getMonth()+3, 0, 23,59,59,999)}; }
function prevQuarterBounds(from){ const s=startOfQuarter(new Date(from.getFullYear(), from.getMonth()-3, 1)); return {start:s, end:new Date(s.getFullYear(), s.getMonth()+3, 0, 23,59,59,999)}; }
function nextSemesterBounds(from){ const s=startOfSemester(new Date(from.getFullYear(), from.getMonth()+6, 1)); return {start:s, end:new Date(s.getFullYear(), s.getMonth()+6, 0, 23,59,59,999)}; }
function prevSemesterBounds(from){ const s=startOfSemester(new Date(from.getFullYear(), from.getMonth()-6, 1)); return {start:s, end:new Date(s.getFullYear(), s.getMonth()+6, 0, 23,59,59,999)}; }
function nextYearBounds(from){ const s=startOfYear(new Date(from.getFullYear()+1,0,1)); return {start:s, end:endOfYear(s)}; }
function prevYearBounds(from){ const s=startOfYear(new Date(from.getFullYear()-1,0,1)); return {start:s, end:endOfYear(s)}; }

function formatPeriodLabel(type, s){
  var d=new Date(s);
  ...
  if(type==='trimestrale'){ var q=Math.floor(d.getMonth()/3)+1; return 'Trimestre '+q+' '+d.getFullYear(); }
...
// — Fix drawer mobile: alto contrasto, testi chiari, tap target grandi
function injectMobileDrawerCSS(){
  ...
  @media (max-width:980px){
    ...
    .drawer button.ghost{ /* nel drawer non vogliamo trasparente */
      background: rgba(255,255,255,.10) !important;
...
// — Mobile LIGHT MODE fixes (solo light mode, solo mobile)
function injectMobileLightModeCSS(){
...
// — Badge / pill ad alto contrasto (LIGHT MODE, mobile+desktop)
function injectLightBadgesCSS(){
...
// ===== FX & Helpers (unici, senza duplicati) =====
function toast(msg){
...
function domFromHTML(h){ var t=document.createElement('div'); t.innerHTML=h; return t.firstElementChild; }

/* ---- SELETTORE GLOBALI E QUERYSTRING ---- */
window.$1   = window.$1   || function(sel){ return document.querySelector(sel); };
window.$all = window.$all || function(sel){ return Array.from(document.querySelectorAll(sel)); };
...
function getQuery(){
  ...
  for(var i=0;i<parts.length;i++){
    ...
    var k = p.split('=')[0];
    var v = p.indexOf('=')>=0 ? p.slice(p.indexOf('=')+1) : '';
    try {
